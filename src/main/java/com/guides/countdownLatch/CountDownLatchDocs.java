package com.guides.countdownLatch;
 ...
 *
 * In this case, we give two examples:
 * 1. Square a bunch of numbers in different threads and store back the example.
 * 2. Merge sort an array using threads and countdown latches.
 */
...
/**
 * This worker squares numbers and saves them in the collector. It initially
 * waits for the startSignal, and then triggers doneSignal once it is over.
 **/
...
/** Threaded Merge Sort using countdown latches */
	...
	public void sort() throws Exception {
		try {
			if (begin < end) {
				...
				while (s1 <= mid && s2 <= end)
					tmp[pos++] = arr[s1] < arr[s2] ? arr[s1++] : arr[s2++];
				while (s1 <= mid)
					tmp[pos++] = arr[s1++];
				while (s2 <= end)
					tmp[pos++] = arr[s2++];
